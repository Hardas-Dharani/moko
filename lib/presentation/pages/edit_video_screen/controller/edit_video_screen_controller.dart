...
class EditVideoController extends GetxController {
  ...
  categoryMenu() async {
    LoadingDialog.show();
    try {
      final s = await ContentCreatorRepositoryIml().channelPlayList();
      channelPlayListModel = ChannelPlayListModel.fromJson(s);
...
// .size (kb)
  ...
  uploadVideo() async {
    LoadingDialog.show();
    try {
      final s = await ContentCreatorRepositoryIml().uploadVideos({
